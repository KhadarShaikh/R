		...
		try {
			...
			if (null != cursor && 0 < cursor.count()) {
				for (DBObject dbObject : cursor) {
					...
					List<RequestResources> qequestResources = (List<RequestResources>) findAllByCondition(mapCondition,
							null);
					/*
					 * DBCursor cursor1 = colls.find(query).skip(0).limit(1); if
					 * (null != cursor1 && cursor1.length() > 0) {
					 * 
					 * @SuppressWarnings("unchecked") T object = (T)
					 * MongoUtil.getObjectByDBObject(dbObject, pojo); if (null
					 * != object) { resultList.add(object); } }
					 */
				}
			}
		} catch (Exception e) {
